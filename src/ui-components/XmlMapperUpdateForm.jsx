...
export default function XmlMapperUpdateForm(props) {
  const {
    id: idProp,
    xmlMapper: xmlMapperModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = xmlMapperRecord
      ? { ...initialValues, ...xmlMapperRecord }
      : initialValues;
    setServiceName(cleanValues.serviceName);
    setFileName(cleanValues.fileName);
    setFilePath(cleanValues.filePath);
    setMapperId(cleanValues.mapperId);
    setMapperName(cleanValues.mapperName);
    setMapperNameSpace(cleanValues.mapperNameSpace);
    setMapperType(cleanValues.mapperType);
    setMapperBody(cleanValues.mapperBody);
    setErrors({});
  };
  const [xmlMapperRecord, setXmlMapperRecord] =
    React.useState(xmlMapperModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(XmlMapper, idProp)
        : xmlMapperModelProp;
      setXmlMapperRecord(record);
    };
    queryData();
  }, [idProp, xmlMapperModelProp]);
  React.useEffect(resetStateValues, [xmlMapperRecord]);
  const validations = {
    serviceName: [],
    fileName: [],
    filePath: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            XmlMapper.copyOf(xmlMapperRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "XmlMapperUpdateForm")}
      {...rest}
    >
      <TextField
        label="Service name"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || xmlMapperModelProp)}
