        ...
        >
            <TableContainer component={Paper}>
                <Table aria-label="simple table">
                    ...
                    <TableBody>
                        {data.map((row, index) => (
                            <TableRow onClick={() => handleOpenModal(row)} key={row.id}>
                                <TableCell>{index + 1}</TableCell>
                                <TableCell>{row.serviceName}</TableCell>
                                <TableCell>{row.mapperType}</TableCell>
                                <TableCell>{row.mapperNameSpace}</TableCell>
                ...
                <TabPanel value={activeTab} index={1}>
                    {selectedRow ? (
                        <UrlListView servicesName={selectedRow.serviceName}
                                     mapperNameSpace={selectedRow.mapperNameSpace}
                                     mapperId={selectedRow.mapperId}
                                     defaultPageSize={10}
                                     setIsModalOpen={setIsModalOpen}/>// 모달시 페이징 안됨...
                    ) : (
